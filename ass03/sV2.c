...
int main(int argc, char *argv[]) {
    ...
    size_t chunkHeight = worldSize / numranks; // Determine chunk size per rank
    size_t realHeight = chunkHeight + 2; // Include space for ghost rows
    g_worldWidth = worldSize; // Set global world width
    ...
    if (myrank == 0) {
        ...
        // Allocation "chunk" of Cell Universe plus "ghost" rows for all ranks
        //g_dataLength = g_worldWidth * realHeight;
        MPI_Scatter(g_data, g_worldWidth * chunkHeight, MPI_UNSIGNED_CHAR,
            g_data + g_worldWidth, g_worldWidth * chunkHeight, MPI_UNSIGNED_CHAR, 0, MPI_COMM_WORLD);
    }

     
    else {
        g_data = calloc(g_worldWidth * (chunkHeight + 2), sizeof(unsigned char)); // +2 for ghost rows        // Other ranks receive their portion of the world
        MPI_Scatter(NULL, 0, MPI_DATATYPE_NULL, 
                g_data + g_worldWidth, g_worldWidth * chunkHeight, MPI_UNSIGNED_CHAR, 
                0, MPI_COMM_WORLD);
    }
